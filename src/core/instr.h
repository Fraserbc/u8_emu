...
struct u8_instr_oper {
	...
	int8_t shift;
	uint16_t flags;
	struct u8_oper (*handler)(struct u8_core *, struct u8_instr_oper *, uint32_t);
};

struct u8_instr {
	void (*handler)(struct u8_core *, uint8_t, struct u8_oper *, struct u8_oper *);
	uint16_t mask;
	uint8_t flags;
...
/* Operand Handlers */
struct u8_oper oper_reg_gp(struct u8_core *core, struct u8_instr_oper *oper, uint32_t val);	// General purpose registers
struct u8_oper oper_reg_ctrl(struct u8_core *core, struct u8_instr_oper *oper, uint32_t val);	// Control registers
struct u8_oper oper_mem(struct u8_core *core, struct u8_instr_oper *oper, uint32_t val);		// Memory access
struct u8_oper oper_imm(struct u8_core *core, struct u8_instr_oper *oper, uint32_t val);		// Immediate value
...
void instr_brel(struct u8_core *core, uint8_t flags, struct u8_oper *op0, struct u8_oper *op1);	// Bits 8-11 used as immediate operand
